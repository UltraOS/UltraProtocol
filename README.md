...
- `module/description` - (string, optional, default=null) - free-form description of the module that the kernel receives via
  the `description` field of `ultra_module_info_attribute`. No description is generated if unset or empty.
...
### ULTRA_MEMORY_TYPE_ACPI_RECLAIMABLE
Memory tagged as ACPI reclaimable by the firmware. Usually contains ACPI tables.
...
- `description` - optional null-terminated ASCII description of the module, as specified in the configuration file

A module only carries a description if one was provided in the configuration file. Whether a description is present can be
tested with the following C macro:

```c
#define ULTRA_MODULE_HAS_DESCRIPTION(header) ((((header).size) - sizeof(struct ultra_module_info_attribute)) > 0)
```

Note that the length of the description cannot be derived from `header.size` because the attribute is padded for alignment,
so `str{n}len()` or similar must be used to determine it.
